...
void MainWindow::on_btnUpdateTable_clicked()
{
    ...
    QList<QVariantList> tableData = {
        // 前两行：PDF中自身学号的前两行
        {2023414300207, "李宝辉", "男", "2023软件卓越2班", "计算机科学与技术学院", "软件工程", "初修"},
        {202105002, "李子诺", "男",  "2023软件卓越2班", "计算机科学与技术学院", "软件工程", "初修"},
        {2023414300211,"刘东波", "男", "2023软件卓越2班", "计算机科学与技术学院", "软件工程", "初修"},
        {202105004, "林俊涛", "男", "2023软件卓越2班", "计算机科学与技术学院", "软件工程", "初修"},
        {202105005, "刘炜东", "男", "2023软件卓越2班", "计算机科学与技术学院", "软件工程", "初修"}
    };
    // 3. 设置表格行数并填充数据
    ui->tableInfo->setRowCount(tableData.size());
    ui->tableInfo->setAlternatingRowColors(ui->chkBoxRowColor->isChecked()); // 保持交替行颜色设置

    for (int row = 0; row < tableData.size(); row++) {
        QVariantList rowData = tableData[row];
        QTableWidgetItem *item = nullptr;

        // 列0：学号
        item = new QTableWidgetItem(QString::number(rowData[0].toUInt()));
        item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
        // 自身行：学号设为粗体红色
        if (row == 2) {
        ...
        ui->tableInfo->setItem(row, 0, item);

        // 列1：姓名
        item = new QTableWidgetItem(rowData[1].toString());
        item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
        // 自身行：姓名设为粗体红色
        if (row == 2) {
            QFont font = item->font();
        ...
        ui->tableInfo->setItem(row, 1, item);

        // 列2：性别（带图标，禁止编辑）
        QIcon icon;
        QString sex = rowData[2].toString();
        ...
        ui->tableInfo->setItem(row, 2, item);

        // 列3：行政班级
        item = new QTableWidgetItem(rowData[3].toString());
        item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
        ui->tableInfo->setItem(row, 3, item);

        // 列4：院系
        item = new QTableWidgetItem(rowData[4].toString());
        item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
        ui->tableInfo->setItem(row, 4, item);

        // 列5：专业
        item = new QTableWidgetItem(rowData[5].toString());
        item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
        ui->tableInfo->setItem(row, 5, item);

        // 列6：修读性质
        item = new QTableWidgetItem(rowData[6].toString());
        item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
        ui->tableInfo->setItem(row, 6, item);
    }
}
//初始化表格数据
